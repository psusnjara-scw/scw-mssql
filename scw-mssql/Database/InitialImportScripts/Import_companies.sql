...
/*
	This stored procedure will import the companies collection
*/
...
SET @FileLocation = 'E:\SCWDemo\2020-01 Mongo Exports\US Export\scw_prod_alpha_api.companies_US.json'
SET @Region = 'US'
...
DROP TABLE IF EXISTS #import_companies;
DROP TABLE IF EXISTS #Company;

-- Create temporary table for bulk insert
CREATE TABLE #import_companies (
	companiesDetails NVARCHAR(MAX)
);

--  Create and execute the statement to import the json file
SET @TableName = '#import_companies'
SET @SQL = 'BULK INSERT ' + @TableName + ' FROM ''' + @FileLocation + ''' WITH (ROWTERMINATOR = ''0x0a'' ); '
SELECT @SQL
EXEC (@SQL)
...
UPDATE #Company
SET CompaniesObject_idHash = HASHBYTES('SHA2_256', CompaniesObject_id)
