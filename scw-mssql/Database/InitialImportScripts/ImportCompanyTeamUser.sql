--CREATE PROCEDURE [dbo].[ImportCompanyTeamUser]
--	@CompanyFileLocation	NVARCHAR(200),
--	@TeamFileLocation		NVARCHAR(200),
--	@UserFileLocation		NVARCHAR(200),
--	@Region					CHAR(2)
--AS

/*
	This stored procedure will import the companies, teams and users collections
*/


DECLARE	@CompanyFileLocation	NVARCHAR(200);
DECLARE	@TeamFileLocation		NVARCHAR(200);
DECLARE	@UserFileLocation		NVARCHAR(200);
DECLARE	@Region					CHAR(2);

SET @CompanyFileLocation = 'E:\SCWDemo\2020-01 Mongo Exports\US Export\scw_prod_alpha_api.companies_US.json'
SET @TeamFileLocation = 'E:\SCWDemo\2020-01 Mongo Exports\US Export\scw_prod_alpha_api.teams_US.json'
SET @UserFileLocation = 'E:\SCWDemo\2020-01 Mongo Exports\US Export\scw_prod_alpha_api.users_US.json'
SET @Region = 'US'
...
--  Create and execute the statement to import the json files
SET @TableName = 'importcompanies'
SET @SQL = 'BULK INSERT ' + @TableName + ' FROM ''' + @CompanyFileLocation + ''' WITH (ROWTERMINATOR = ''0x0a'' ); '
SELECT @SQL
EXEC (@SQL)

SET @TableName = 'importteams'
SET @SQL = 'BULK INSERT ' + @TableName + ' FROM ''' + @TeamFileLocation + ''' WITH (ROWTERMINATOR = ''0x0a'' ); '
SELECT @SQL
EXEC (@SQL)

SET @TableName = 'importusers'
SET @SQL = 'BULK INSERT ' + @TableName + ' FROM ''' + @UserFileLocation + ''' WITH (ROWTERMINATOR = ''0x0a'' ); '
SELECT @SQL
EXEC (@SQL)

--  Perform companies processing first
CREATE TABLE [#Company] (
    [CompaniesObject_id]		NVARCHAR(40)			NULL,
	[CompaniesObject_idHash]	VARBINARY(8000)			NULL,
    [SsoErrorMessage]			NVARCHAR(20)			NULL,  
	[AccountOwner]				NVARCHAR(100)			NULL,
    [ExpirationDate]			NVARCHAR(MAX)			NULL, 
	[ExpirationDate_DEC]		DECIMAL(18,1)			NULL,
...
UPDATE #Company
SET ExpirationDate_DEC = (ROUND(CAST (CAST(ExpirationDate AS FLOAT) AS DECIMAL(18,1)),0) / 60000 ),
	CompaniesObject_idHash = HASHBYTES('SHA2_256', CompaniesObject_id)
